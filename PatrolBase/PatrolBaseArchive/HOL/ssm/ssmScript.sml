...
`configuration = 
 CFG
  ...
  (('state -> ('command option,'principal,'d,'e)Form))
...
`CFGInterpret
 ...
 (CFG 
  ...
  (stateInterp:'state -> ('command option,'principal,'d,'e)Form)
...
Hol_reln
`(!(elementTest:('command option,'principal,'d,'e)Form -> bool)
    (NS: 'state -> ('command list) trType -> 'state) M Oi Os Out (s:'state)
    (certList:('command option,'principal,'d,'e)Form list)
    (stateInterp:'state -> ('command option,'principal,'d,'e)Form)
    (x:('command option,'principal,'d,'e)Form list)
 ...
 (CFGInterpret (M,Oi,Os)
  (CFG elementTest stateInterp certList (x::ins) s outs)) ==>
 (TR 
  ((M:('command option,'b,'principal,'d,'e)Kripke),Oi:'d po,Os:'e po)
  (exec (commandList x))
  (CFG elementTest stateInterp certList (x::ins) s outs)
  (CFG elementTest stateInterp certList ins
       (NS s (exec (commandList x)))
       ((Out s (exec (commandList x)))::outs)))) /\
 (!(elementTest:('command option,'principal,'d,'e)Form -> bool)
    (NS: 'state -> ('command list) trType -> 'state) M Oi Os Out (s:'state)
    (certList:('command option,'principal,'d,'e)Form list)
    (stateInterp:'state -> ('command option,'principal,'d,'e)Form)
 ...
 (CFG elementTest stateInterp certList (x::ins) s outs)
 (CFG elementTest stateInterp certList ins
      (NS s (trap (commandList x)))
      ((Out s (trap (commandList x)))::outs)))) /\
 (!(elementTest:('command option,'principal,'d,'e)Form -> bool)
    (NS: 'state -> ('command list) trType -> 'state) M Oi Os Out (s:'state)
    (certList:('command option,'principal,'d,'e)Form list)
    (stateInterp:'state -> ('command option,'principal,'d,'e)Form)
 ...
 (TR 
  ((M:('command option,'b,'principal,'d,'e)Kripke),Oi:'d po,Os:'e po)
  (discard (commandList x))
 (CFG elementTest stateInterp certList (x::ins) s outs)
 (CFG elementTest stateInterp certList ins
      (NS s (discard (commandList x)))
      ((Out s (discard (commandList x)))::outs))))`
...
TAC_PROOF(([],
``!elementTest certList stateInterp (x:('command option,'principal,'d,'e)Form list)
   ins s outs.
 (!M Oi Os. 
 (CFGInterpret
  ((M :('command option, 'b, 'principal, 'd, 'e) Kripke),(Oi :'d po), (Os :'e po))
  (CFG elementTest
       (stateInterp:'state -> ('command option,'principal,'d,'e)Form) certList
       (x::ins)
 ...
 TR
  ...
  (CFG (elementTest :('command option, 'principal, 'd, 'e) Form -> bool)
	   (stateInterp:'state -> ('command option,'principal,'d,'e)Form)
           ...
           (s :'state) (outs :'output list))
   (CFG elementTest stateInterp certList ins
           ((NS :'state -> 'command list trType -> 'state) s (exec (commandList x)))
           (Out s (exec (commandList x))::outs)) <=>
  (authenticationTest elementTest x) /\
  (CFGInterpret (M,Oi,Os)
   (CFG elementTest stateInterp certList (x::ins) s outs)) /\
 ...
 TR
  ...
  (CFG (elementTest :('command option, 'principal, 'd, 'e) Form -> bool)
	   (stateInterp:'state -> ('command option,'principal,'d,'e)Form)
           ...
           (s :'state) (outs :'output list))
   (CFG elementTest stateInterp certList ins
           ((NS :'state -> 'command list trType -> 'state) s (trap (commandList x)))
           (Out s (trap (commandList x))::outs)) <=>
  (authenticationTest elementTest x) /\
  (CFGInterpret (M,Oi,Os)
   (CFG elementTest stateInterp certList (x::ins) s outs)) /\
